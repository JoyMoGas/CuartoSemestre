...
def crea_diccionario_solo_anio(lista_peliculas: list) -> list:
    listap = []
    hoy = datetime.now()
    for pelicula in lista_peliculas:
        estreno = pelicula['fecha_estreno']
        # Convertir la fecha de estreno a objeto datetime
        estreno = datetime.strptime(estreno, "%Y/%m/%d")
        # Extraer solo el año de la fecha de estreno
        anio_estreno = estreno.strftime("%Y")
        # Guardar solo el año en el diccionario de la película
