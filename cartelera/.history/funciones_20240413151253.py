...
def crea_diccionario_solo_anios(lista_peliculas: list) -> dict:
    anios = {}  # Utilizamos un diccionario para almacenar el recuento de películas por año
    for pelicula in lista_peliculas:
        fecha_estreno = pelicula.get('fecha_estreno')
        if fecha_estreno:
            anio = fecha_estreno.split('/')[0]  # Obtenemos el año de la fecha de estreno
            # Incrementamos el contador para el año actual o lo inicializamos a 1 si es nuevo
            if anio in anios:
                anios[anio] += 1
            else:
                anios[anio] = 1
