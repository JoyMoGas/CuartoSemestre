import os
from juego import Juego  # Corrected import
...
def run():
    ...
    if opcion == 1:
        ...
        juego = Juego(Tablero(), Jugador(name1, "X", ["X", "O"]), Jugador(name2, "O", ["X", "O"]))  # Added instantiation
        juego.game_cycle(name1, name2)

    elif opcion == 2:  # Corrected condition
        nameP = input("Ingresa tu username:")
        juego = Juego(Tablero(), Jugador(nameP, "X", ["X", "O"]), Jugador("Máquina", "O", ["X", "O"]))  # Added instantiation
