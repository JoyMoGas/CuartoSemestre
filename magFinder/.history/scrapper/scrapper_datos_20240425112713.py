            ...
            if journalgrid:
                ...
                if len(elements) >= 6:  # Adjusted number of elements to match the new fields count
                    data.update({
                        ...
                        'SUBJECT AREA AND CATEGORY': elements.get_text(strip=True),
                        'PUBLISHER': elements.get_text(strip=True),
                        'H-INDEX': elements.get_text(strip=True),
                        'PUBLICATION TYPE': elements.get_text(strip=True),
                        'ISSN': elements.get_text(strip=True),
                        'COVERAGE': elements.get_text(strip=True)
                    })
