...
def peliculas_cartelera(lista_peliculas:list)->list:
    '''Regresa las películas de más reciente estreno'''
    listap = []
    for pelicula in lista_peliculas:
        #print(pelicula['titulo']," - ",pelicula['fecha_estreno'])
        estreno = pelicula['id']
        pelicula['titulo'] = estreno
    lista_sorted = sorted(lista_peliculas, key=lambda x: x['titulo'],reverse=False)
    listap = lista_sorted
    return listap
...
def crea_diccionario_pelicula(lista_peliculas:list) -> dict:
    d = {}
...
def crea_diccionario_solo_genero(lista_peliculas:list) -> dict:
    d = {}
    for pelicula in lista_peliculas:
        keys = pelicula["genero"]
        keys = unicodedata.normalize('NFD', keys).encode('ascii', 'ignore').decode('utf-8')
